...
async function openProjectSourcesUI(env, args) {
  ...
  if (env.page === void 0) {
    const boot2 = await bootstrap(env, bootstrapArgsForProject(project.url, args));
    if (!boot2.ok) {
      return boot2;
    }
  }
...
function bootstrapArgsForProject(url, args) {
  ...
  if (args.preferExistingTab !== void 0) {
    boot2.preferExistingTab = args.preferExistingTab;
  }
  if (args.existingTab === true) {
    boot2.existingTab = {
      target: { type: "url", url },
    ...
    boot2.existingTab = args.existingTab;
  }
  return boot2;
}
...
async function readProjectSourcesSnapshot(page) {
  if (typeof page.evaluate === "function") {
    try {
      const raw = await page.evaluate(() => {
        const normalize2 = (value) => value.replace(/\s+/g, " ").trim();
        const textOf = (element) => normalize2(element.innerText ?? element.textContent ?? "");
        ...
        const sourceNameFromCandidate = (text) => {
          const normalized = normalize2(text).replace(/\s+(Document|File|PDF|Image|Spreadsheet|Text|Code|CSV|Markdown)\s+·.*$/i, "");
        ...
        const sourcesFromSection = Array.from(document.querySelectorAll("section[aria-label]")).filter((section) => /sources?/i.test(section.getAttribute("aria-label") ?? "") && section.closest("[role='dialog'], [role='menu']") === null).flatMap((section) => Array.from(section.querySelectorAll("[aria-label], button")).flatMap((element) => {
          ...
          const aria = normalize2(element.getAttribute("aria-label") ?? "");
        ...
        const candidates = Array.from(document.querySelectorAll("[role='tab'], button, a")).map((element) => {
          const label = normalize2(element.getAttribute("aria-label") ?? element.getAttribute("title") ?? textOf(element));
...
var requiredScenarios = [
  ...
  scenario("chat-work-expansion", true, () => true, async (context, meta) => {
    ...
    const details = {};
    let booted = false;
    let terminal = syntheticCommand(meta.startedAt);
    let failure;
    try {
      const boot2 = await chatgpt.session.bootstrap({ preferExistingTab: false, timeoutMs: 6e4 });
      terminal = asCommand(boot2);
      requireLiveCommand("session.bootstrap", boot2);
      booted = true;
      const openedChat = await chatgpt.experience.open({ experience: "chat", timeoutMs: 6e4 });
      terminal = asCommand(openedChat);
      requireLiveCommand(
        "experience.open.chat",
        openedChat,
      ...
      requireLiveCommand(
        "configuration.apply.chat.noop",
        verifiedChatConfiguration,
        verifiedChatConfiguration.ok && verifiedChatConfiguration.data?.verified === true
      );
      const openedWork = await chatgpt.experience.open({ experience: "work", timeoutMs: 6e4 });
      terminal = asCommand(openedWork);
      requireLiveCommand(
        "experience.open.work",
        openedWork,
      ...
      requireLiveCommand(
        "configuration.inspect.work",
        workConfiguration,
        workConfiguration.ok && workConfiguration.data?.experience === "work" && workConfiguration.data.verified === true && hasAxes(workConfiguration.data, ["model", "effort", "speed"])
      );
      const workDesired = activeSelection(workConfiguration.data, ["model", "effort", "speed"]);
      requireLiveCommand(
        "configuration.inspect.work.active",
        workConfiguration,
      ...
      requireLiveCommand(
        "configuration.apply.work.noop",
        verifiedWorkConfiguration,
      ...
      const started = await chatgpt.work.start({
        ...
        newTask: true,
        wait: { timeoutMs: 18e4, stableMs: 1500, pollMs: 750 },
      ...
      requireLiveCommand(
        "responses.create.work",
        responseResult,
        response.object === "chatgpt.browser.response" && response.status === "ok" && textEquals(response.output_text, "WORK_EXPANSION_RESPONSES_OK")
      );
    ...
    return failure === void 0 ? pass(meta, terminal, details) : fail(meta, failure.command, { ...details, failedStage: failure.stage });
  }),
...
var optionalScenarios = [
  scenario("configuration-mutate-restore", false, (context) => contextEnvFlag(context, "CHATGPT_E2E_CONFIGURATION_MUTATION"), async (context, meta) => {
    ...
    try {
      ...
      requireLiveCommand("session.bootstrap", boot2);
      booted = true;
      const opened = await chatgpt.experience.open({ experience: "work", timeoutMs: 6e4 });
      ...
      requireLiveCommand(
        "configuration.inspect.work.alternative",
        inspected,
        originalEffort !== void 0 && alternative !== void 0
      );
      restoreNeeded = true;
      ...
      requireLiveCommand(
        "configuration.apply.work.mutate",
        changed,
  ...
  scenario("running-status-detection", false, (context) => contextEnvFlag(context, "CHATGPT_E2E_RUNNING_STATUS"), async (context, meta) => {
    ...
    try {
      ...
      if (!asked.ok) return fail(meta, asked);
      await waitForGenerationSignal(env, 3e4);
      ...
      return status.ok && (data?.completionState === "generating" || data?.generationActive === true) ? pass(meta, status, details) : fail(meta, status, details);
    } finally {
      await stopGenerationIfVisible(env);
    }
  }),
  scenario("download-generated-file", false, (context) => contextEnvFlag(context, "CHATGPT_E2E_DOWNLOAD"), async (context, meta) => {
    const env = await bootNewThread(context, meta);
    if ("status" in env) return env;
    const chat = await openExperience(env, { experience: "chat", timeoutMs: 6e4 });
    ...
    const details = { path: path3, bytes, suggestedFilename: download?.suggestedFilename, exactFile, exactContent, askStatus: asked.status };
...
function generatedFileAskCanProceed(result) {
  return result.ok || result.status === "partial" && result.data?.generationActive !== true;
}
...
function includesUploadedFilename(actual, expected) {
  ...
  const stem = escapeRegExp4(expected.slice(0, extensionIndex).toLowerCase());
  const extension = escapeRegExp4(expected.slice(extensionIndex).toLowerCase());
...
var LiveSmokeCommandFailure = class extends Error {
  constructor(stage, command) {
...
function requireLiveCommand(stage, command, condition = command.ok) {
  if (!condition) {
...
function syntheticCommand(timestamp) {
  return {
    ok: true,
    status: "ok",
    warnings: [],
...
function activeSelection(inspection, axes) {
  const selection = {};
...
function hasSelectionAxes(selection, axes) {
  return axes.every((axis) => selection[axis] !== void 0);
}
function configurationDetails(inspection) {
  return {
