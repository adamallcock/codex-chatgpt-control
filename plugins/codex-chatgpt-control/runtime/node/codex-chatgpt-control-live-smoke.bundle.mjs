...
import { mkdtemp, readFile as readFile2, stat as stat4, writeFile as writeFile3 } from "node:fs/promises";
...
var optionalScenarios = [
  scenario("download-generated-file", false, (context) => contextEnvFlag(context, "CHATGPT_E2E_DOWNLOAD"), async (context, meta) => {
    ...
    const bytes = path === void 0 ? 0 : (await stat4(path).catch(() => void 0))?.size ?? 0;
...
async function tempFile(name, body) {
  const dir = await mkdtemp(join4(tmpdir(), "chatgpt-live-smoke-"));
