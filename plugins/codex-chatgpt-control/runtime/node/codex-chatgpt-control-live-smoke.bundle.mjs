...
import { mkdtemp, readFile as readFile2, stat as stat5, writeFile as writeFile4 } from "node:fs/promises";
...
import { copyFile, mkdir as mkdir3, stat as stat2, writeFile as writeFile2 } from "node:fs/promises";
...
async function saveLatestVisibleImageSource(page, destDir, timeoutMs) {
  ...
  const path = join3(absoluteDest, suggestedFilename);
  await writeFile2(path, parsed.bytes);
...
async function doctor(env, args = {}) {
  ...
  const checks = {};
  const boot2 = await bootstrap(env, { preferExistingTab: true, timeoutMs: 3e4 });
...
function bridgeCheck(boot2) {
  if (boot2.blocker?.kind === "browser_bridge_unavailable") {
    return blocked(boot2.blocker.message, bridgeRemediation(boot2));
  }
...
async function loginCheck(env, boot2) {
  if (!boot2.ok && boot2.blocker?.kind === "login_required") {
...
import { mkdir as mkdir4, stat as stat4, writeFile as writeFile3 } from "node:fs/promises";
...
async function createRunReport(env, result, options = {}) {
  try {
    ...
    const path = join4(destDir, `${stamp}-${safeBase}.json`);
    ...
    await writeFile3(path, `${JSON.stringify(report2, null, 2)}
`, "utf8");
...
var requiredScenarios = [
  ...
  scenario("redacted-run-report", true, () => true, async (context, meta) => {
    ...
    const body = path === void 0 ? "" : await readFile2(path, "utf8").catch(() => "");
    return result.ok && body.includes("[redacted:") && !body.includes("private@example.com") && !body.includes("/example/user/private") ? pass(meta, result, { path }) : fail(meta, result, { path, bodyPreview: body.slice(0, 500) });
  }),
  ...
  scenario("runner-report-redacted", true, () => true, async (context, meta) => {
    ...
    const body = path === void 0 ? "" : await readFile2(path, "utf8").catch(() => "");
    return result.ok && path !== void 0 && body.includes("[redacted:") && !body.includes(secret) ? pass(meta, result, { path }) : fail(meta, result, { path, bodyPreview: body.slice(0, 500), output: result.output_text });
  }),
...
var optionalScenarios = [
  scenario("download-generated-file", false, (context) => contextEnvFlag(context, "CHATGPT_E2E_DOWNLOAD"), async (context, meta) => {
    ...
    const path = typeof result.data === "object" && result.data !== null ? result.data.path : void 0;
    const bytes = path === void 0 ? 0 : (await stat5(path).catch(() => void 0))?.size ?? 0;
...
function includesUploadedFilename(actual, expected) {
  ...
  const stem = escapeRegExp2(expected.slice(0, extensionIndex).toLowerCase());
  const extension = escapeRegExp2(expected.slice(extensionIndex).toLowerCase());
...
function stepPreviewText(steps, id) {
...
function includesStep(steps, id) {
